...
class Api {
    ...
    async postProduct(req, res) {
        ...
        if (productoNuevo.name && productoNuevo.price && productoNuevo.photo && 
            productoNuevo.desc && productoNuevo.code && productoNuevo.stock && productoNuevo.timestamp && productoNuevo.pw === "admin" && Object.keys(productoNuevo).length === 8) {
            const objetos = await this.getAll();
            if (objetos.length) {
    ...
    async putProduct(req, res) {
        ...
        prodMod.desc && prodMod.code && prodMod.stock && prodMod.timestamp && prodMod.pw === "admin" && Object.keys(prodMod).length === 8 ? true : null;
